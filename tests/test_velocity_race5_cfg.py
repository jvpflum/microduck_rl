...
def test_race5_penalizes_world_cross_track_velocity_and_allows_recovery():
    ...
    assert cfg.terminations["race_out_of_lane"].params["lane_half_width_m"] == 0.75
    assert cfg.terminations["race_heading_departure"].params["maximum_yaw_rad"] > 0.6
...
def test_race5_v14_uses_a_static_objective_and_champion_smoothness():
    ...
    assert cfg.rewards["race_speed_squared"].weight == 4.5
    assert cfg.rewards["race_lane_error"].weight == -22.0
    assert cfg.rewards["race_world_lateral_speed"].weight == -13.0
    assert cfg.rewards["race_heading_error"].weight == -22.0
